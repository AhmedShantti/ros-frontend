 ...
 * signed-on employee's open session, and `components/terminal/pos-live.tsx`
 * checks this record against it as soon as a PIN sign-on is known,
 * overwriting it (or clearing it) with whatever the server answers. Storage
 * alone can never be trusted over that: a deploy or a cleared browser loses
 * it outright, and a stale value here — from a session someone else already
 * closed — must not be replayed as if it were still open.
 *
