...
export const treasury = {
  ...
  /** `GET /cash-sessions/current` — DEMO-CASH-SESSION-RECOVERY-P0 — the caller's OWN open cash session at THEIR OWN terminal-bound branch, if exactly one exists. Recovery contract: after a fresh PIN login (e.g. after a frontend reload/deploy wiped the locally-remembered `cashSessionId`), the POS calls this BEFORE showing Open Shift. A non-null `cashSession` means the employee already holds an open shift here — the client resumes it in-place; `POST /cash-sessions` is never called again for it, and no new drawer is opened. `null` means either no open session exists (normal Open Shift) or more than one does (ambiguous — see `CashSessionsService.findCurrentForEmployee`). Gated on `cash.session.open` — the SAME permission `POST /cash-sessions` already requires, deliberately not a new/invented read permission. This mirrors `GET /cash-sessions/drawers` immediately above: a narrow read of state the caller may already act on, scoped to their own terminal-derived branch and their own employee identity, never a generic CashSession read (`findOne` stays internal-only — see its own docblock and D-20). — The caller's own open cash session at their own terminal-bound branch, or null if none (or more than one) exists. */
  getCurrentSession: () =>
  ...
  recountClose: (sessionId: string, body: S.RecountCashSessionCloseDto) =>
    http.post<S.TreasuryController_recountCloseResponse>("/cash-sessions/{sessionId}/close/recount", { params: { sessionId }, body, idempotent: true }),
  ...
  listOpenSessions: (branchId: string) =>
    http.get<S.OpenCashSessionsController_listOpenSessionsResponse>("/branches/{branchId}/cash-sessions/open", { params: { branchId } }),
...
export const api = { sales, auth, tenants, rbac, terminals, password, organisation, governance, catalogue, production, treasury, workforceEmployees, workforceSchedules, workforceAttendance, inventory, health, kitchen, reporting, sync };
