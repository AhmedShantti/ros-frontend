...
export interface CreateDrawerDto {
  ...
  name: string;
  /** Optional LEGACY device binding — `DrawersService.create` still enforces same-branch (ADR 0008 D-16) when a value is supplied. CROSSCUT-POS-KDS- TERMINAL-DECOUPLING-P0 (2026-09-13): opening a cash session no longer checks this field at all — POS is a branch/employee-scoped application session with no terminal identity, so a drawer bound to a terminal is selectable from any POS session at the same branch exactly as an unbound drawer is. Retained only as administrative/hardware-inventory metadata for tenants that still track it. */
  terminalId?: string;
}
...
export interface CreateMenuItemDto {
  ...
  taxClassId?: string;
  /** The item's sellable variant(s), created atomically with the item in one transaction — never a second "add variant" step just to make the item sellable. The common single-variant case sends a one-element array. */
...
export interface CreateServiceChargePolicyDto {
  /** Omitted = effective immediately (resolved to DATABASE time, never this process's clock). A past instant is rejected — enforced by the DB CHECK (`ck_scp_no_backdating`), not merely this validator. */
  effectiveFrom?: string;
...
export interface CreateSupplierItemLinkDto {
...
export interface CreateSupplierPriceEntryDto {
  currency: string;
  packSize: string;
  purchaseUnitId: string;
...
export interface CreateVariantDto {
  ...
  name: Record<string, unknown>;
  prepTimeSeconds?: number;
  /** Every variant is directly sellable from creation — no Price List concept. */
  price: string;
  sortOrder?: number;
}
...
export interface DecidePurchaseOrderDto {
  /** CROSSCUT-POS-KDS-TERMINAL-DECOUPLING-P0 §9 — a Purchase Order may be tenant-wide (warehouse/central-kitchen delivery), so no single unambiguous branch is always derivable from the order itself. This is OPERATIONAL identity context ONLY — the branch the approving employee's PIN/membership is verified against — never a device identity. Never the branch a Purchase Order is attributed or delivered to. */
...
export interface PurchaseOrderLineDto {
  ...
  purchaseUnitId?: string;
  quantity?: string;
  /** Present to consolidate from a submitted requisition line; when set, `stockItemId`/`purchaseUnitId`/`quantity`/`attributionBranchId` are all taken from the requisition line and MUST NOT also be supplied. */
  sourceRequisitionLineId?: string;
  stockItemId?: string;
  taxAmount?: string;
  /** Explicit, authorised negotiated price (minor units). Omit to resolve the current agreed SupplierPriceEntry automatically (mission brief §3). */
  unitPrice?: string;
}
...
export interface UpdateSupplierItemLinkDto {
  isActive?: boolean;
  preferenceRank?: number;
  supplierBarcodes?: string[];
  supplierItemCode?: string;
...
export interface UpsertSettingValueDto {
...
/** `GET /branches/{branchId}/cash-sessions/open` — Every OPEN or CLOSING cash session at this branch, oldest first — enough to identify and act on a stranded session through the existing close-other workflow (close-context / close / close/finalize). */
export type OpenCashSessionsController_listOpenSessionsResponse = ({
  branchId: string;
...
/** `GET /cash-sessions/current` — DEMO-CASH-SESSION-RECOVERY-P0 — the caller's OWN open (or closing) cash session at THEIR OWN POS session branch, if exactly one exists. Recovery contract: after a fresh PIN login (e.g. after a frontend reload/deploy wiped the locally-remembered `cashSessionId`), the POS calls this BEFORE showing Open Shift. A non-null `cashSession` means the employee already holds a session here — the client resumes it in-place; `POST /cash-sessions` is never called again for it, and no new drawer is opened. A `status: "closing"` session (CASH-SESSION-RESUME-AND-CLOSE-P0) means it is frozen awaiting a manager's finalize decision — the client routes back to the close flow, not ordinary POS. `null` means either no such session exists (normal Open Shift) or more than one does (ambiguous — see `CashSessionsService.findCurrentForEmployee`). Gated on `cash.session.open` — the SAME permission `POST /cash-sessions` already requires, deliberately not a new/invented read permission. This mirrors `GET /cash-sessions/drawers` immediately above: a narrow read of state the caller may already act on, scoped to their own session-derived branch and their own employee identity, never a generic CashSession read (`findOne` stays internal-only — see its own docblock and D-20). — The caller's own open or closing cash session at their own POS session branch, or null if none (or more than one) exists. */
...
/** `GET /cash-sessions/drawers` — DEMO-OPS-HOTFIX-3 — the real drawers a Cashier may open a shift over, for the POS Open-Shift drawer selector. Resolves the drawer list from the CALLER'S OWN live-verified POS session branch (`DrawersService.listForBranch`), never a caller-supplied branchId — a cashier cannot browse another branch's drawers by asking for one. Gated on `cash.session.open`, the SAME permission `POST /cash-sessions` already requires, deliberately NOT `settings.branch.manage` — this is a read of what a Cashier may already act on, not a drawer-administration grant (that lives on the separate `DrawersController`). — The caller's own POS session branch's drawers. */
...
export type TreasuryController_getCloseContextResponse = {
  ...
  status: "open" | "closing" | "closed";
  /** The approval threshold this session closes under — in BOTH blind and open mode. "0" for a legacy session that opened before the branch had any cash-close policy (default controls apply). */
  toleranceMinorUnits: string;
  /** Present only once status is closing/closed. */
  varianceMinorUnits: string;
};

/** `POST /cash-sessions/{sessionId}/close/finalize` — The manager's decision on a frozen (above-tolerance) close — FR-FIN-006 [M], FR-SEC-016/030/032/033. The manager PIN is verified BEFORE the business transaction opens (`identity/contract`'s `APPROVER_PIN_VERIFIER` — a failed-attempt/ lockout counter must survive a later rollback, and never runs at all on an idempotent replay). The verified manager's permission set — not the calling cashier's — is what `cash.variance.approve` is checked against, by the Approval Runtime itself, never by a route-level permission guard (the approver is a different actor than the caller). R-6(a): an explicit REJECTED decision COMMITS and returns 200 with `outcome: "rejected"` — never an error. The session stays `closing`; a retry supplies FRESH `approvalRequestId`/`approvalDecisionId` values. — The manager decision outcome — "closed" (approved) or "rejected" (R-6(a); the session remains closing — the cashier may then POST .../close/recount). */
...
/** `GET /catalogue/completeness` — C-11 completeness report: what would block sellability, without blocking anything itself. The pricing half of C-11 no longer applies — every variant carries a mandatory direct price from creation, so "unpriced variant" is structurally impossible. */
...
/** `POST /catalogue/items` — Create a menu item AND its sellable variant(s) atomically, in one call — no separate "add variant" step is needed to make it sellable. — The newly created menu item, with its newly created variants (each already priced). */
export type CatalogueController_createItemResponse = {
  ...
  variants: ({
    ...
    /** Direct sellable price — no Price List concept. */
    price: string;
    sortOrder: number;
  })[];
};
...
export type CatalogueController_updateVariantPriceBody = UpdateVariantPriceDto;
...
export type PlatformSettingsController_upsertBranchResponse = void;

export type PlatformSettingsController_upsertBranchBody = UpsertSettingValueDto;

/** `DELETE /platform/settings/branch/{branchId}/{settingKey}` */
export type PlatformSettingsController_unsetBranchResponse = void;

/** `PUT /platform/settings/brand/{brandId}/{settingKey}` */
export type PlatformSettingsController_upsertBrandResponse = void;

export type PlatformSettingsController_upsertBrandBody = UpsertSettingValueDto;

/** `DELETE /platform/settings/brand/{brandId}/{settingKey}` */
export type PlatformSettingsController_unsetBrandResponse = void;

/** `GET /platform/settings/inspect` — The FR-PLT-027 level-by-level breakdown. */
export type PlatformSettingsController_inspectResponse = void;

/** `GET /platform/settings/resolve` — The FR-PLT-025 effective value and lock metadata. */
export type PlatformSettingsController_resolveResponse = void;

/** `PUT /platform/settings/tenant/{settingKey}` */
export type PlatformSettingsController_upsertTenantResponse = void;

export type PlatformSettingsController_upsertTenantBody = UpsertSettingValueDto;

/** `DELETE /platform/settings/tenant/{settingKey}` */
export type PlatformSettingsController_unsetTenantResponse = void;

/** `PUT /platform/settings/terminal/{terminalId}/{settingKey}` */
export type PlatformSettingsController_upsertTerminalResponse = void;

export type PlatformSettingsController_upsertTerminalBody = UpsertSettingValueDto;

/** `DELETE /platform/settings/terminal/{terminalId}/{settingKey}` */
export type PlatformSettingsController_unsetTerminalResponse = void;

/** `GET /procurement/comparative-pricing` */
export type ProcurementController_comparativePricingResponse = ({
  currency: string;
  /** Decimal quantity as a string (preserves exact precision). */
  packSize: string;
  preferenceRank: number;
  purchaseUnitId: string;
  selectedTier: Record<string, unknown> | null;
  supplierCode: string;
  supplierId: string;
  ...
  volumeTiers: (Record<string, unknown>)[] | null;
})[];

/** `GET /procurement/purchase-orders` */
export type PurchaseOrdersController_listResponse = ({
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
  deliveryLocationType: "branch" | "warehouse" | "central_kitchen";
  expectedDeliveryDate: string;
...
export type PurchaseOrdersController_createResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_createBody = CreatePurchaseOrderDto;

/** `GET /procurement/purchase-orders/{id}` */
export type PurchaseOrdersController_getByIdResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_updateResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_updateBody = UpdatePurchaseOrderDto;

/** `POST /procurement/purchase-orders/{id}/amend` */
export type PurchaseOrdersController_amendResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_amendBody = AmendPurchaseOrderDto;

/** `GET /procurement/purchase-orders/{id}/amendments` */
export type PurchaseOrdersController_listAmendmentsResponse = (Record<string, unknown>)[];

/** `POST /procurement/purchase-orders/{id}/approve` */
export type PurchaseOrdersController_approveResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_approveBody = DecidePurchaseOrderDto;

/** `POST /procurement/purchase-orders/{id}/reject` */
export type PurchaseOrdersController_rejectResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_rejectBody = DecidePurchaseOrderDto;

/** `POST /procurement/purchase-orders/{id}/submit` */
export type PurchaseOrdersController_submitResponse = {
  approvalBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  approvedBand: "auto" | "tier_1" | "tier_2" | "tier_3" | null | null;
  currency: string;
  deliveryLocationId: string;
...
export type PurchaseOrdersController_submitBody = SubmitPurchaseOrderDto;

/** `GET /procurement/requisitions` */
export type RequisitionsController_listResponse = ({
  createdAt: string;
  id: string;
  lines: (Record<string, unknown>)[];
  notes: string | null;
...
export type RequisitionsController_createResponse = {
  createdAt: string;
  id: string;
  lines: (Record<string, unknown>)[];
  notes: string | null;
...
export type RequisitionsController_createBody = CreateRequisitionDto;

/** `GET /procurement/requisitions/{id}` */
export type RequisitionsController_getByIdResponse = {
  createdAt: string;
  id: string;
  lines: (Record<string, unknown>)[];
  notes: string | null;
...
export type RequisitionsController_submitResponse = {
  createdAt: string;
  id: string;
  lines: (Record<string, unknown>)[];
  notes: string | null;
...
/** `GET /procurement/supplier-item-links` */
export type ProcurementController_listSourcingLinksResponse = ({
  createdAt: string;
  id: string;
  isActive: boolean;
  preferenceRank: number;
...
export type ProcurementController_createSourcingLinkResponse = {
  createdAt: string;
  id: string;
  isActive: boolean;
  preferenceRank: number;
...
export type ProcurementController_createSourcingLinkBody = CreateSupplierItemLinkDto;

/** `PATCH /procurement/supplier-item-links/{id}` */
export type ProcurementController_updateSourcingLinkResponse = {
  createdAt: string;
  id: string;
  isActive: boolean;
  preferenceRank: number;
...
export type ProcurementController_updateSourcingLinkBody = UpdateSupplierItemLinkDto;

/** `GET /procurement/supplier-price-entries` */
export type ProcurementController_priceHistoryResponse = ({
  createdAt: string;
  currency: string;
  id: string;
  /** Decimal quantity as a string (preserves exact precision). */
...
export type ProcurementController_createPriceEntryResponse = {
  createdAt: string;
  currency: string;
  id: string;
  /** Decimal quantity as a string (preserves exact precision). */
...
export type ProcurementController_createPriceEntryBody = CreateSupplierPriceEntryDto;

/** `GET /procurement/supplier-price-entries/effective` */
export type ProcurementController_effectivePriceResponse = ({
  createdAt: string;
  currency: string;
  id: string;
  /** Decimal quantity as a string (preserves exact precision). */
...
export type ProcurementController_listSuppliersResponse = ({
...
export type ProcurementController_createSupplierResponse = {
  addresses: (Record<string, unknown>)[];
  code: string;
  contacts: (Record<string, unknown>)[];
  createdAt: string;
...
export type ProcurementController_createSupplierBody = CreateSupplierDto;

/** `GET /procurement/suppliers/{id}` */
export type ProcurementController_getSupplierResponse = {
  addresses: (Record<string, unknown>)[];
  code: string;
  contacts: (Record<string, unknown>)[];
  createdAt: string;
...
export type ProcurementController_updateSupplierResponse = {
  addresses: (Record<string, unknown>)[];
  code: string;
  contacts: (Record<string, unknown>)[];
  createdAt: string;
...
export type ProcurementController_updateSupplierBody = UpdateSupplierDto;

/** `PATCH /procurement/suppliers/{id}/status` */
export type ProcurementController_setSupplierStatusResponse = {
  addresses: (Record<string, unknown>)[];
  code: string;
  contacts: (Record<string, unknown>)[];
  createdAt: string;
...
export type ServiceChargePolicyController_createBranchPolicyResponse = {
  createdAt: string;
  createdBy: string;
  effectiveFrom: string;
  id: string;
  ...
  rules: ({
    ...
    /** null = applies to all order types. */
    orderType: "dine_in" | "takeaway" | "delivery" | "drive_thru" | "pickup" | "aggregator" | null | null;
...
export type ServiceChargePolicyController_createBranchPolicyBody = CreateServiceChargePolicyDto;

/** `POST /service-charge-policy/brand/{brandId}` — The newly created brand-level service-charge policy version. */
export type ServiceChargePolicyController_createBrandPolicyResponse = {
  createdAt: string;
  createdBy: string;
  effectiveFrom: string;
  id: string;
...
export type ServiceChargePolicyController_createBrandPolicyBody = CreateServiceChargePolicyDto;
...
export type ServiceChargePolicyController_resolveResponse = void;

/** `POST /service-charge-policy/tenant` — The newly created tenant-level service-charge policy version. */
export type ServiceChargePolicyController_createTenantPolicyResponse = {
  createdAt: string;
  createdBy: string;
  effectiveFrom: string;
  id: string;
...
export type ServiceChargePolicyController_createTenantPolicyBody = CreateServiceChargePolicyDto;

/** `GET /service-charge-policy/versions` — Every version for ONE exact scope, newest first — including future-scheduled ones `resolve` cannot show. See `ServiceChargePolicyService.listVersions`'s own docblock for why this minimal surface exists. — Every version for the given (level, targetId) scope, newest first. */
export type ServiceChargePolicyController_listVersionsResponse = void;

/** `DELETE /service-charge-policy/versions/{versionId}` — Cancel a still-future version. `RequireAnyPermission` here is a COARSE gate only — the actual, scope-precise authorization decision happens inside `ServiceChargePolicyService.cancel`, after the row's own level is known (see that method's docblock). */
export type ServiceChargePolicyController_cancelResponse = void;
...
export const ROUTES = {
  ...
  OpenCashSessionsController_listOpenSessions: { method: "GET", path: "/branches/{branchId}/cash-sessions/open" },
  ...
  CatalogueController_updateVariantPrice: { method: "PATCH", path: "/catalogue/variants/{variantId}/price" },
  ...
  PlatformSettingsController_upsertBranch: { method: "PUT", path: "/platform/settings/branch/{branchId}/{settingKey}" },
  PlatformSettingsController_unsetBranch: { method: "DELETE", path: "/platform/settings/branch/{branchId}/{settingKey}" },
  PlatformSettingsController_upsertBrand: { method: "PUT", path: "/platform/settings/brand/{brandId}/{settingKey}" },
  PlatformSettingsController_unsetBrand: { method: "DELETE", path: "/platform/settings/brand/{brandId}/{settingKey}" },
  PlatformSettingsController_inspect: { method: "GET", path: "/platform/settings/inspect" },
  PlatformSettingsController_resolve: { method: "GET", path: "/platform/settings/resolve" },
  PlatformSettingsController_upsertTenant: { method: "PUT", path: "/platform/settings/tenant/{settingKey}" },
  PlatformSettingsController_unsetTenant: { method: "DELETE", path: "/platform/settings/tenant/{settingKey}" },
  PlatformSettingsController_upsertTerminal: { method: "PUT", path: "/platform/settings/terminal/{terminalId}/{settingKey}" },
  PlatformSettingsController_unsetTerminal: { method: "DELETE", path: "/platform/settings/terminal/{terminalId}/{settingKey}" },
  ProcurementController_comparativePricing: { method: "GET", path: "/procurement/comparative-pricing" },
  PurchaseOrdersController_list: { method: "GET", path: "/procurement/purchase-orders" },
  PurchaseOrdersController_create: { method: "POST", path: "/procurement/purchase-orders" },
  PurchaseOrdersController_getById: { method: "GET", path: "/procurement/purchase-orders/{id}" },
  PurchaseOrdersController_update: { method: "PATCH", path: "/procurement/purchase-orders/{id}" },
  PurchaseOrdersController_amend: { method: "POST", path: "/procurement/purchase-orders/{id}/amend" },
  PurchaseOrdersController_listAmendments: { method: "GET", path: "/procurement/purchase-orders/{id}/amendments" },
  PurchaseOrdersController_approve: { method: "POST", path: "/procurement/purchase-orders/{id}/approve" },
  PurchaseOrdersController_reject: { method: "POST", path: "/procurement/purchase-orders/{id}/reject" },
  PurchaseOrdersController_submit: { method: "POST", path: "/procurement/purchase-orders/{id}/submit" },
  RequisitionsController_list: { method: "GET", path: "/procurement/requisitions" },
  RequisitionsController_create: { method: "POST", path: "/procurement/requisitions" },
  RequisitionsController_getById: { method: "GET", path: "/procurement/requisitions/{id}" },
  RequisitionsController_submit: { method: "POST", path: "/procurement/requisitions/{id}/submit" },
  ProcurementController_listSourcingLinks: { method: "GET", path: "/procurement/supplier-item-links" },
  ProcurementController_createSourcingLink: { method: "POST", path: "/procurement/supplier-item-links" },
  ProcurementController_updateSourcingLink: { method: "PATCH", path: "/procurement/supplier-item-links/{id}" },
  ProcurementController_priceHistory: { method: "GET", path: "/procurement/supplier-price-entries" },
  ProcurementController_createPriceEntry: { method: "POST", path: "/procurement/supplier-price-entries" },
  ProcurementController_effectivePrice: { method: "GET", path: "/procurement/supplier-price-entries/effective" },
  ProcurementController_listSuppliers: { method: "GET", path: "/procurement/suppliers" },
  ProcurementController_createSupplier: { method: "POST", path: "/procurement/suppliers" },
  ...
  ServiceChargePolicyController_createTenantPolicy: { method: "POST", path: "/service-charge-policy/tenant" },
  ServiceChargePolicyController_listVersions: { method: "GET", path: "/service-charge-policy/versions" },
  ServiceChargePolicyController_cancel: { method: "DELETE", path: "/service-charge-policy/versions/{versionId}" },
