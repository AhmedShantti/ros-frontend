...
export function LivePos() {
  ...
  /*
   * PROD-POS-SESSION-RECOVERY-P0 is handled by the `sync()` subscription in
   * the mount effect above, which replaced the standalone listener that used
   * to live here. Same defect — an already-mounted tree kept rendering a
   * cashier and a drawer that storage no longer backed, while every request
   * 401'd underneath it — and the same trigger, `onSessionChange`. The
   * difference is that `sync()` re-READS rather than only nulling out, so it
   * covers signing ON as well as falling off, and picks up the drawer record
   * too. `isSignedIn()` is still what decides whether anyone is on the till.
   *
   * The terminal listing that used to be fetched here is gone deliberately:
   * `GET /auth/terminals` is the tenant-wide admin endpoint and a bound POS
   * token is not entitled to call it, so it 401'd — and every 401 trips this
   * client's own refresh, which made even a successful sign-on look broken.
   * The branch comes from `scope.branchId`, as it does everywhere else on
   * this screen.
   */
